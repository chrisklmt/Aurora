...
internal fun privateChatWifiDirectDebugDiagnostics(
    ...
    val blockedReason = when {
        ...
        !guard.receiveBridgeEnabled -> "Enable the Wi-Fi Direct receive bridge on the receiver."
        ...
        else -> "Private Wi-Fi Direct routing not enabled."
    }

    return WifiDirectPrivateDebugDiagnostics(
        guard = guard,
        readiness = WifiDirectPrivateDebugReadiness(
            overallStatus = if (guard.prerequisitesReady) "not wired yet" else "not ready",
            pathStatus = "BLE only",
