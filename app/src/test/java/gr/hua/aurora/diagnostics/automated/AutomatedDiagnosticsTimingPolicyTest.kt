...
class AutomatedDiagnosticsTimingPolicyTest {
    ...
    fun defaultTimingPolicyMatchesPhaseTwoDefaults() {
        ...
        assertEquals(12_000L, policy.wifiDirectDiscovery.timeoutMillis)
        assertEquals(1_000L, policy.wifiDirectGroupFormation.stabilizationMillis)
        assertEquals(12_000L, policy.wifiDirectGroupFormation.timeoutMillis)
        assertEquals(1, policy.wifiDirectGroupFormation.maxRetries)
        assertEquals(800L, policy.wifiDirectSocketConnection.stabilizationMillis)
        assertEquals(8_000L, policy.wifiDirectSocketConnection.timeoutMillis)
        assertEquals(2, policy.wifiDirectSocketConnection.maxRetries)
        assertEquals(600L, policy.wifiDirectAdapterReadiness.stabilizationMillis)
        assertEquals(5_000L, policy.wifiDirectAdapterReadiness.timeoutMillis)
        assertEquals(300L, policy.wifiDirectBridgeEnable.stabilizationMillis)
        assertEquals(3_000L, policy.wifiDirectBridgeEnable.timeoutMillis)
        assertEquals(300L, policy.hybridControlDelivery.stabilizationMillis)
        assertEquals(5_000L, policy.hybridControlDelivery.timeoutMillis)
        assertEquals(500L, policy.hybridSocketHintDelivery.stabilizationMillis)
        assertEquals(5_000L, policy.hybridSocketHintDelivery.timeoutMillis)
        assertEquals(300L, policy.hybridBootstrapTrigger.stabilizationMillis)
        assertEquals(5_000L, policy.hybridBootstrapTrigger.timeoutMillis)
